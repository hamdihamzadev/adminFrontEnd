...
const actions = {
    ...
    async ac_addCategory({commit},category){
        try{
            const token = localStorage.getItem('token')
            const response = await axios.post(`${apiUrl}/api/categoryProduct/newCategory`,category, {
                headers: {
                    Authorization: `Bearer ${token}`,
    ...
    async ac_updateCategory({commit},{id,formCategoryUpdate}) {
        try {
            const token = localStorage.getItem('token')
            const response= await axios.put(`${apiUrl}/api/categoryProduct/category/${id}`,formCategoryUpdate, {
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'multipart/form-data'
                }
